...
const getFixedPosition = (windowKey, screenWidth, screenHeight) => {
  // Standard window dimensions for calculation - increased for better visibility
  ...
  finalX = Math.max(
    margin,
    Math.min(finalX, screenWidth - windowWidth - margin),
  );
  finalY = Math.max(
    margin,
    Math.min(finalY, screenHeight - windowHeight - margin),
  );
...
const WindowWrapper = (Component, windowKey) => {
  const Wrapped = (props) => {
    const { focusWindow, windows, activeWindow, dockOrigins } =
      useWindowStore();
