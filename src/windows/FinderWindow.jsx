...
const Finder = () => {
  ...
  const visitLink = isProject
    ? activeLocation.children.find((c) => ["url", "link"].includes(c.fileType))
        ?.href
    : null;
  ...
  return (
    <>
      <div
        id="window-header"
        className="flex items-center p-0 overflow-hidden h-12"
      >
      ...
      <div className="bg-white flex h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden">
          {isProject ? (
            /* Project Detail View */
            <div className="h-full overflow-y-auto p-6 pb-16 bg-white">
              <div className="max-w-5xl mx-auto space-y-3">
                {/* Project Images - Conditional Layout Based on Project Type */}
                {activeLocation.projectType === "mobile" ? (
                  /* Mobile App: 4 Portrait Images in a Row */
                  <div className="w-full flex gap-8 justify-center">
                    {(activeLocation.previewImages || [])
                      .slice(0, 4)
                      .map((img, index) => (
                        <button
                          key={index}
                          type="button"
                          className="w-[16%] aspect-[9/19.5] bg-gray-100 rounded-lg overflow-hidden border border-gray-200 shadow-sm cursor-pointer hover:border-blue-400 transition-colors flex-shrink-0"
                          onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                  ...
                  /* Website: 2 Landscape Images Side by Side */
                  <div className="w-full grid grid-cols-2 gap-3">
                    {(
                      activeLocation.previewImages || [
                        activeLocation.previewImage,
